import { getOneBookFromDb } from "../../db/use-cases/Book.js";

export const viewBook = async (httpRequest) => {
  if (areParamsValid(httpRequest)) {
    const response = await getOneBookFromDb(httpRequest.params);
...
function checkForNullResponse(response) {
  if (response === null) {
    ...
    error.message = "Book with given _id not found";
    throw error;
  }
}
